const SugarJournal = {
	...
	created() {
		var vm = this;
		requirejs(
			["sugar-web/activity/activity", "sugar-web/env", "lz-string"],
			function (activity, env, LZString) {
	...
	methods: {
		saveData: function (context) {
			var compressedData = this.LZString.compressToUTF16(
		...
		insertFromJournal: function (type) {
			...
			return new Promise((resolve, reject) => {
				requirejs(
					...
					function (datastore, journalchooser) {
						setTimeout(function () {
							journalchooser.show(
								function (entry) {
									if (!entry) {
